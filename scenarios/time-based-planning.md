...
**Hire Date** — When a new position starts (person joins or vacancy is filled). The position appears in Forecast headcount and cost starting from this date.

**Termination Date** — When a person leaves or a position is eliminated. The position no longer appears in headcount after this date; costs are removed from projections.
...
For positions created in a scenario (no "before" state), effective dates control **when** the position appears in Forecast. There is no "before" location — the position only ever appears in its assigned department, starting from the effective date.

## Common Questions

**What if I don't set an effective date?** The change is treated as immediate — it appears in the "after" state for all Forecast time periods.

**Can I set different effective dates on different positions in the same scenario?** Yes — this is the standard approach for phased reorgs.

**How far can I forecast?** Up to 5 years from the current date.

**Do effective dates work for all field types?** Yes — department, location, manager, title, salary, and any custom fields.

**If I move someone with an effective date, do they appear in their old department in the org chart?** No — the org chart shows the "after" state. Select "Show Before" to see the original state. Forecast is where you see the time-phased view.
