---
description: Long-term headcount and budget planning
icon: calendar-days
hidden: true
---

# Multi-Year Planning

> **Note: This page is under construction. Content coming soon!**

## Planned Content Outline

- Setting up multi-year forecasts (2-3+ years)
- Annual vs multi-year planning considerations
- Handling uncertainty in long-term projections
- Growth assumptions and hiring ramps over multiple years
- Modeling compound growth and scale
- Budget projections across fiscal years
- Scenario planning within multi-year forecasts
- Updating multi-year forecasts with actual data
- Rolling forecasts vs static multi-year plans
- Communicating multi-year plans to leadership
- Best practices for long-term workforce planning
- When to refresh and adjust multi-year forecasts

---

For immediate assistance, please contact [support@agentnoon.com](mailto:support@agentnoon.com) or refer to [Support & How to Self-Help](../start-here/support-self-help.md).
