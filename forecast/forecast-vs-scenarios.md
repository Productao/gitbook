...
**Forecast = visualization** — A view-only module that displays workforce data as time-based projections. Shows headcount and cost aggregated by department, location, or any attribute, over monthly/quarterly/yearly periods. Does not make changes.

**Scenarios = planning** — Editable workspaces where you add, close, move, and modify positions. Tracks before/after impact, supports approval workflows, and isolates changes from Main Org. Does not produce time-based aggregations on its own.

**Together:** Create a scenario with changes and hire dates → view the scenario in Forecast → see the time-phased impact.
...
1. **Plan in Scenarios** — Add positions, close positions, move teams, set hire/effective dates
2. **Project in Forecast** — Switch to Forecast view within the scenario; set aggregator, time period, and metric; export
3. **Approve in Scenarios** — Submit via OpEx Panel; approvers review and approve/reject
4. **Report in Forecast** — Export final approved projections for Finance, HR, and leadership
