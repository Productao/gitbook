---
description: Budget planning and cost tracking in Forecast
icon: dollar-sign
hidden: true
---

# Budget Planning & Tracking

> **Note: This page is under construction. Content coming soon!**

## Planned Content Outline

- Linking headcount forecasts to budget planning
- Setting budget targets by department and quarter
- Calculating total compensation costs in forecasts
- Including fully-loaded costs (benefits, taxes, overhead)
- Tracking actuals vs budget throughout the year
- Identifying budget variances and adjustments
- What-if budget scenarios (increase/decrease by X%)
- Forecasting non-headcount costs
- Budget approval workflows in Forecast
- Reporting budget status to finance teams
- Budget reforecasting and adjustments mid-year
- Best practices for budget accuracy

---

For immediate assistance, please contact [support@agentnoon.com](mailto:support@agentnoon.com) or refer to [Support & How to Self-Help](../start-here/support-self-help.md).
