---
description: Generating reports and exporting forecast data
icon: file-download
hidden: true
---

# Forecast Reports & Exports

> **Note: This page is under construction. Content coming soon!**

## Planned Content Outline

- Available report types in Forecast
- Generating headcount summary reports
- Budget vs actuals reports
- Departmental forecast reports
- Exporting forecast data to Excel/CSV
- Exporting forecast charts and visuals
- Creating executive summary reports
- Scheduled reports and automation
- Custom report templates
- Sharing reports with stakeholders outside Agentnoon
- Report versioning and archiving
- Best practices for forecast reporting cadence

---

For immediate assistance, please contact [support@agentnoon.com](mailto:support@agentnoon.com) or refer to [Support & How to Self-Help](../start-here/support-self-help.md).
