...
**Image (PNG)** — Static snapshot of the current view. Best for embedding in documents or sharing via Slack/email.

## How to Export

1. Configure Forecast view: row aggregator (Department, Location, Employee Type), time period (Monthly/Quarterly/Yearly), year, and metric (Headcount or Cost)
...
**Quarterly headcount report** (for HR/Recruiting): Aggregator = Department, Time Period = Quarterly, Metric = Headcount, View = Show After

**Annual budget report** (for Finance/CFO): Aggregator = Department, Time Period = Yearly, Metric = Cost, add Bonus/Stock if needed

**Geographic cost distribution** (for COO/Finance): Aggregator = Location or Country, Yearly, Cost

**Scenario impact report** (for approvers): Open scenario, Aggregator = Department, Time Period = Quarterly, **View = Show Changes** (shows +/- deltas), export Cost and Headcount separately

**5-year strategic plan** (for CEO/Board): Aggregator = Department, Yearly (shows all 5 years automatically), Cost

**Hiring plan** (for Recruiting): Open scenario with new positions + hire dates, Monthly, Headcount, Show Changes — filter positive changes in Excel for the new-hire timeline
