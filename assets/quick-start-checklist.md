---
description: New user onboarding checklist
icon: clipboard-check
hidden: true
---

# Quick Start Checklist

> **Note: This page is under construction. Content coming soon!**

## Planned Content Outline

- Day 1 checklist: first login, profile setup, basic navigation
- Week 1 checklist: exploring Main Org, understanding data, watching tutorials
- First scenario checklist: creating, editing, sharing a test scenario
- Admin onboarding checklist (for admins)
- Department lead checklist (for managers)
- Planner checklist (for workforce planners)
- Quarterly planning checklist
- Annual planning checklist
- Downloadable PDF checklist
- Tracking progress through onboarding

---

For immediate assistance, please contact [support@agentnoon.com](mailto:support@agentnoon.com) or refer to [Support & How to Self-Help](../start-here/support-self-help.md).
